        ...
        private async void BtnStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    while (true)
                    {
                        UpdateLog($"📖 กำลังกวาดข้อมูลสิ่งปฏิกูล หน้าที่ {pageCounter}...");
                        ...
                        // 🎯 ด่านตรวจการเปลี่ยนหน้า: ดักจับปุ่ม Next ของตารางปลายทาง
                        var nextButton = page.Locator("#waste_pro_table1_wrapper a[data-dt-idx='next']"); ;
                        bool hasNextPage = await nextButton.CountAsync() > 0;
                        if (hasNextPage)
                        {
                            // 🎯 ไม้ตาย: ดึงค่าคุณสมบัติ aria-disabled ออกมาเช็กสด ๆ เลยครับพี่ยัง
                            string ariaDisabled = await nextButton.GetAttributeAsync("aria-disabled") ?? "false";

                            // 🔍 ถ้าพบว่าค่าของมันกลายเป็น "true" แปลว่าหมดหน้าสุดท้ายแล้วจริง ๆ ครับพี่!
                            if (ariaDisabled.Equals("true", StringComparison.OrdinalIgnoreCase))
                            {
                                hasNextPage = false;
                            }

                            // ดักเช็กเสริมความปลอดภัยเผื่อปุ่มหลุดหายไปจากหน้าจอ
                        ...
                        if (!hasNextPage)
                        {
                            UpdateLog("✨ สิ้นสุดด่านตรวจ! บอทกวาดข้อมูลครบทุกหน้ายันหน้าสุดท้ายเรียบร้อยแล้ว");
                            break;
                        }

                        // 🚀 ถ้าผ่านด่านมาได้ แปลว่าลุยต่อได้! สั่งคลิกปุ่ม Next ของพี่ได้เลย
                        UpdateLog($"➡️ ตารางยังมีหน้าถัดไป... กำลังสั่งคลิกปุ่ม Next (หน้า {pageCounter} -> หน้า {pageCounter + 1})");
                        await nextButton.ClickAsync();
                        pageCounter++;

                        // หน่วงเวลา 1.5 วินาที รอให้ระบบเว็บปลายทางเรนเดอร์ข้อมูลตารางหน้าถัดไปให้เสร็จก่อนขึ้นลูปใหม่
